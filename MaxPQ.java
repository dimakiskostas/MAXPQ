...
//            int result = cmp.compare(heap[i], heap[p]);  //compare parent with child
            int result = ((Comparable<T>) heap[i]).compareTo(heap[p]);  //compare parent with child
            if (result <= 0) {
    ...
    private void sink(int i) {
        ...
        while (left <= size) {
            ...
            if (right <= size) {
                max = (((Comparable<T>) heap[left]).compareTo(heap[right])) < 0 ? right : left;
            }
            // If the heap condition holds, stop. Else swap and go on.
            if ( ((Comparable<T>) heap[i]).compareTo(heap[max]) >= 0) {
                return;
            }
            swap(i, max);
            i = max;
